...
public class SnowflakeIdWorker {
    ...
    private static String getIp() {
        ...
        try {
            ...
            throw new AppException("获得ip异常", "500");
        }
    ...
    public synchronized long nextId() {
        ...
        if (this.workerId <= 0) {
            setDataAndWork();
            log.info("dataId:{} workId:{}", datacenterId, workerId);
        }
    ...
    private void setDataAndWork() {
        if (redisUtils != null) {
            String workId = RedisKeys.getPermsKey(getIp() + ":workId");
            String datacenterId = RedisKeys.getPermsKey(getIp() + ":datacenterId");
            String increment = redisUtils.get(workId);
            if (increment == null) {
                String permsKey = RedisKeys.getPermsKey("workId:datacenterId:incr:");
                long wd = redisUtils.increment(permsKey, 1L);

                //重新排序开始
                if (wd > 961L) {
                    wd = 1L;
                    redisUtils.set(permsKey, wd);
                }
                this.workerId = wd / 31 + 1;
                this.datacenterId = wd % 31;
                redisUtils.set(workId, this.workerId + "", RedisUtils.NOT_EXPIRE);
                redisUtils.set(datacenterId, this.datacenterId + "", RedisUtils.NOT_EXPIRE);

            } else {
                this.workerId = Long.valueOf(increment);
                this.datacenterId = Long.valueOf(redisUtils.get(datacenterId));
            }
        } else {
            this.workerId = IdWorkerFactory.getWorkerId();
            this.datacenterId = IdWorkerFactory.getDataCenterId();
        }
    }
